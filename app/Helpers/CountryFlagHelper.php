...
class CountryFlagHelper
{
    ...
    /**
     * Render chỉ cờ (không kèm tên đội)
     */
    public static function renderFlagOnly(?string $teamName, string $classes = 'w-full h-full object-cover'): string
    {
        if (empty($teamName)) {
            return '';
        }
        ...
        $fallbackSvg = '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="'.$classes.' text-gray-400"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>';
        ...
        try {
            return svg('flag-4x3-'.$iso, $classes)->toHtml();
        } catch (\Exception $e) {
            return $fallbackSvg;
        }
    }
